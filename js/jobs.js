...
//   bridge   — ANY SLC-area early-career job to float me right after moving (in-person /
//            hybrid / remote-from-SLC); no special certs/licenses I don't have; low pay OK.
const KNOWN_TRACKS = ["remote", "west", "slc", "bridge", "programs", "field"];
...
const TRACK_BLURB = {
  ...
  bridge: "Any SLC-area early-career job to float me the first few months after moving — in-person, hybrid, or remote-from-SLC. No special certs/licenses required; pay floor is low (flagged). Not career roles — just steady income while I apply for those.",
...
const TRACK_BADGE = {
  ...
  bridge: `<span class="badge" style="background:#567d8c;color:var(--white);">🌉 SLC Bridge</span>`,
...
const TRACK_EMPTY = {
  ...
  bridge: { h: "No SLC bridge jobs yet", p: "Any SLC-area early-career job to float me after moving (no special certs, low pay OK) shows up here when <code>/discover-jobs</code> finds them." },
